# Range report prompt v1

Write an English technical time-range report from `INPUT.json`. Return JSON matching
...
- `content` is the time-range report people read first. It must be brief and scannable.
...
`userNote` is an explicit correction or clarification written by the user after reviewing
the imported context. Preserve it and use it to correct the summary. Do not silently
override measured evidence with an unsupported user claim.

If a block has no `intentTrace` entries, use its `supportingDraft` as the source. Rewrite
the supplied claims into concise bullets and return an empty `details` array. Do not
invent implementation detail that is absent from the draft.

The application appends References to detail pages. Do not write a References section.

## Layer 1: concise time-range report
...
1. `content` is a bullet-first time-range report, not a paper.

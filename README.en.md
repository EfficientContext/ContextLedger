...
notes into concise time-range reports backed by PostgreSQL.

The report has two layers:

- a short, bullet-first update for status updates;
...
MCP server with detected coding agents, installs IntentTrace and `ctx`, and starts the web
app.
...
# Sync visible Codex and Claude Code session context through IntentTrace
...
Run `ctx open`, then use the Context page to:

- sync Codex and Claude Code sessions for an explicit date range;
- browse the project timeline;
- inspect the IntentTrace graph, validation, claims, and references;
- edit your own Context and save a revision;
- add a correction that the next report will read.
